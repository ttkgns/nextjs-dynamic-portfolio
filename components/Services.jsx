import { assets, personalData, serviceData } from '@/assets/assets'
...
const Services = () => {
  return (
    ...
    id='services' className='max-w-5xl mx-auto max-md:px-10 py-10 scroll-mt-20'>
      <motion.h4 
      ...
      className='text-center max-w-2xl mx-auto mt-5 mb-12 font-ovo'>{personalData.servicesIntroduction}</motion.p>
      <motion.div
      initial={{opacity:0}}
      whileInView={{opacity:1}}
        ...
        {serviceData.map(({icon,title,description,link},index) => (
          <motion.div whileHover={{scale:1.05}} key={index} className='border border-gray-400 rounded-lg px-8 py-12 hover:shadow-black cursor-pointer hover:bg-lightHover hover:-translate-y-1 duration-500 dark:hover:bg-darkHover dark:hover:shadow-white'>
            ...
            <a href={link} className='flex items-center gap-2 text-sm mt-5'>
              Read more <Image src={assets.right_arrow} alt='arrow' className='w-4 mt-0.5' />
            </a>
          </motion.div>
        ))}
      </motion.div>
